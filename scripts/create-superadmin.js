...
const possibleEnvPaths = [
  ...
  path.resolve(process.cwd(), "apps/api/.env.production"),
...
try {
  ...
  try {
    PrismaClient = require(path.resolve(process.cwd(), "apps/api/node_modules/@prisma/client")).PrismaClient;
  } catch (err) {
    console.error("❌ Não foi possível carregar o PrismaClient. Verifique se as dependências foram instaladas.");
    process.exit(1);
  }
}
...
try {
  ...
  try {
    bcrypt = require(path.resolve(process.cwd(), "apps/api/node_modules/bcrypt"));
  } catch (err) {
    console.error("❌ Não foi possível carregar o bcrypt. Verifique se as dependências foram instaladas.");
    process.exit(1);
  }
}
